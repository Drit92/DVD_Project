...
def engineer_features(app_data, prev_data):
    """EXACTLY Colab Parts 1+2: All feature engineering"""
    ...
    refusal_flag = prev_data[prev_data['NAME_CONTRACT_STATUS'] == 'Refused']['SK_ID_CURR'].unique()
    app_data['FLAG_EVER_REFUSED'] = app_data['SK_ID_CURR'].apply(lambda x: 1 if x in refusal_flag else 0)
    ...
    app_data['FIN_SCORE'] = app_data['CREDIT_BIN'].map(credit_points).fillna(0)
    
    app_data['BEHAV_SCORE'] = app_data['FLAG_EVER_REFUSED'] * 2
    ext_points = {'Q1 (low)': 3, 'Q2': 2, 'Q3': 1, 'Q4 (high)': 0}
    app_data['EXT_SCORE'] = app_data['EXT2_Q'].map(ext_points).fillna(0)
    
    app_data['RISK_SCORE'] = (app_data['FIN_SCORE'] + app_data['BEHAV_SCORE'] + app_data['EXT_SCORE']).astype(int)
    ...
    return clean_df.sample(50000)  # Lightweight: 50k rows for Streamlit
